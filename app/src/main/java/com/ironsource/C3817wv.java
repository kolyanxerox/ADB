package com.ironsource;

import android.annotation.SuppressLint;
import android.os.Handler;
import android.webkit.WebResourceError;
import android.webkit.WebResourceRequest;
import android.webkit.WebSettings;
import android.webkit.WebView;
import android.webkit.WebViewClient;
import com.google.android.gms.internal.measurement.o0OO0O0;
import com.ironsource.C3056dv;
import com.ironsource.mediationsdk.C3404p;
import com.ironsource.mediationsdk.IronSource;
import com.ironsource.mediationsdk.testSuite.TestSuiteActivity;
import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;
import org.json.JSONException;
import org.json.JSONObject;

/* renamed from: com.ironsource.wv */
/* loaded from: classes2.dex */
public final class C3817wv implements InterfaceC3235iv {

    /* renamed from: a */
    private final Handler f12543a;

    /* renamed from: b */
    private WebView f12544b;

    /* renamed from: c */
    private final WebView f12545c;

    /* renamed from: d */
    private final JSONObject f12546d;

    /* renamed from: e */
    private boolean f12547e;

    /* renamed from: f */
    private boolean f12548f;

    /* renamed from: g */
    private final InterfaceC3078ei f12549g;

    /* renamed from: com.ironsource.wv$a */
    public static final class a extends WebViewClient {

        /* renamed from: b */
        final /* synthetic */ String f12551b;

        public a(String str) {
            this.f12551b = str;
        }

        @Override // android.webkit.WebViewClient
        public void onPageFinished(WebView webView, String str) throws JSONException {
            if (!C3817wv.this.f12547e && str != null && o00O0oO.o0000O00.Oooo0OO(str, String.valueOf(this.f12551b), false)) {
                C3817wv c3817wv = C3817wv.this;
                c3817wv.m12985a(c3817wv.m12983a("onDataReady", o0OO0O0.OooOO0o(c3817wv.f12546d)));
                C3096ev.f8463a.m8567d();
                C3817wv.this.f12547e = true;
            }
            super.onPageFinished(webView, str);
        }

        @Override // android.webkit.WebViewClient
        public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) throws JSONException {
            kotlin.jvm.internal.OooOo.OooO0o0(view, "view");
            kotlin.jvm.internal.OooOo.OooO0o0(request, "request");
            kotlin.jvm.internal.OooOo.OooO0o0(error, "error");
            C3096ev.f8463a.m8564a(error.toString());
            C3573qv.f11160a.m11682b("got error during TestSuite web controller loading " + error);
            super.onReceivedError(view, request, error);
        }
    }

    /* renamed from: com.ironsource.wv$b */
    public static final class b implements InterfaceC3490oi {

        /* renamed from: b */
        final /* synthetic */ InterfaceC3490oi f12553b;

        public b(InterfaceC3490oi interfaceC3490oi) {
            this.f12553b = interfaceC3490oi;
        }

        /* renamed from: a */
        public static final void m12989a(b this$0) {
            kotlin.jvm.internal.OooOo.OooO0o0(this$0, "this$0");
            this$0.onUIReady();
        }

        @Override // com.ironsource.InterfaceC3490oi
        public void onClosed() {
            this.f12553b.onClosed();
        }

        @Override // com.ironsource.InterfaceC3490oi
        public void onUIReady() {
            if (C3817wv.this.f12548f) {
                this.f12553b.onUIReady();
            } else {
                C3817wv.this.f12543a.postDelayed(new OooO0o(this, 15), 500L);
            }
        }
    }

    public C3817wv(TestSuiteActivity activity, InterfaceC3490oi uiLifeCycleListener, JSONObject jsonData, String str) {
        kotlin.jvm.internal.OooOo.OooO0o0(activity, "activity");
        kotlin.jvm.internal.OooOo.OooO0o0(uiLifeCycleListener, "uiLifeCycleListener");
        kotlin.jvm.internal.OooOo.OooO0o0(jsonData, "jsonData");
        this.f12543a = new Handler(activity.getMainLooper());
        this.f12544b = new WebView(activity);
        this.f12545c = new WebView(activity);
        this.f12546d = jsonData;
        this.f12549g = C3495on.f10667s.m11213d().mo8694t();
        m12975a(activity, uiLifeCycleListener, str);
    }

    /* renamed from: e */
    public static final void m12982e(C3817wv this$0) {
        kotlin.jvm.internal.OooOo.OooO0o0(this$0, "this$0");
        this$0.f12548f = true;
    }

    /* renamed from: b */
    public final void m12986b() {
        WebView webView = this.f12544b;
        if (webView != null) {
            webView.destroy();
            this.f12544b = null;
        }
    }

    /* renamed from: c */
    public final WebView m12987c() {
        return this.f12545c;
    }

    /* renamed from: d */
    public final WebView m12988d() {
        return this.f12544b;
    }

    /* renamed from: a */
    public final String m12983a(String methodName, List<? extends Object> methodArgs) {
        kotlin.jvm.internal.OooOo.OooO0o0(methodName, "methodName");
        kotlin.jvm.internal.OooOo.OooO0o0(methodArgs, "methodArgs");
        String strConcat = "TestSuiteJs.".concat(methodName);
        String strM12832b = C3783vv.f12423a.m12832b(methodArgs);
        if (strM12832b.length() <= 0) {
            return OooO0oO.OooOo.OooOoo0(strConcat, "()");
        }
        return strConcat + '(' + strM12832b + ')';
    }

    /* renamed from: a */
    public final void m12984a() {
        m12986b();
        this.f12545c.destroy();
    }

    @SuppressLint({"SetJavaScriptEnabled", "AddJavascriptInterface"})
    /* renamed from: a */
    private final void m12975a(TestSuiteActivity testSuiteActivity, InterfaceC3490oi interfaceC3490oi, String str) {
        WebView webView = this.f12544b;
        WebSettings settings = webView != null ? webView.getSettings() : null;
        if (settings != null) {
            settings.setJavaScriptEnabled(true);
        }
        this.f12545c.getSettings().setJavaScriptEnabled(true);
        ArrayList arrayListOooOOoo = o00O0.OooOo.OooOOoo(IronSource.AD_UNIT.BANNER, IronSource.AD_UNIT.INTERSTITIAL, IronSource.AD_UNIT.REWARDED_VIDEO);
        if (this.f12549g.mo8493a() != null) {
            Iterator<IronSource.AD_UNIT> it = C3404p.m10470j().m10556m().iterator();
            while (it.hasNext()) {
                arrayListOooOOoo.remove(it.next());
            }
        } else {
            arrayListOooOOoo.clear();
        }
        this.f12545c.addJavascriptInterface(new C3201hv(new C3056dv.a().m8404a(arrayListOooOOoo, testSuiteActivity, this.f12543a), new b(interfaceC3490oi), this), "TestSuiteBridge");
        this.f12545c.setWebViewClient(new a(str));
        if (str != null && str.length() != 0) {
            this.f12545c.loadUrl(str);
        }
        WebView webView2 = this.f12544b;
        if (webView2 != null) {
            webView2.loadUrl("javascript:document.write(atob(''))");
        }
        this.f12543a.postDelayed(new OooO0o(this, 13), 2000L);
        this.f12543a.postDelayed(new OooO0o(interfaceC3490oi, 14), 5000L);
    }

    /* renamed from: a */
    public static final void m12976a(InterfaceC3490oi uiLifeCycleListener) {
        kotlin.jvm.internal.OooOo.OooO0o0(uiLifeCycleListener, "$uiLifeCycleListener");
        uiLifeCycleListener.onUIReady();
    }

    /* renamed from: a */
    public static final void m12977a(C3817wv this$0, String script) {
        kotlin.jvm.internal.OooOo.OooO0o0(this$0, "this$0");
        kotlin.jvm.internal.OooOo.OooO0o0(script, "$script");
        this$0.f12545c.loadUrl("javascript:".concat(script));
    }

    /* renamed from: a */
    public final void m12985a(String script) {
        kotlin.jvm.internal.OooOo.OooO0o0(script, "script");
        this.f12543a.post(new o00000O(3, this, script));
    }

    @Override // com.ironsource.InterfaceC3235iv
    /* renamed from: a */
    public void mo9395a(String callbackName, IronSource.AD_UNIT adUnit, List<? extends Object> args) {
        kotlin.jvm.internal.OooOo.OooO0o0(callbackName, "callbackName");
        kotlin.jvm.internal.OooOo.OooO0o0(adUnit, "adUnit");
        kotlin.jvm.internal.OooOo.OooO0o0(args, "args");
        if (this.f12547e) {
            String string = adUnit.toString();
            kotlin.jvm.internal.OooOo.OooO0Oo(string, "adUnit.toString()");
            m12985a(m12983a("onCallbackReceived", o00O0.OooOo.OooOOo(callbackName, string, args)));
        }
    }
}
