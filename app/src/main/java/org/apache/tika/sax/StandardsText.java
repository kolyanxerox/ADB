...
public class StandardsText {
    ...
    static {
        ...
        REGEX_STANDARD = OooOo.OooOO0o(".*", organzationsRegex, ".+", organzationsRegex, "?.*");
    }
    ...
    private static ArrayList<StandardReference> findStandards(String str, Map<Integer, String> map, double d) {
        ...
        while (matcher.find()) {
            StandardReference.StandardReferenceBuilder secondOrganization = new StandardReference.StandardReferenceBuilder(matcher.group("mainOrganization"), matcher.group(Constants.IDENTIFIER)).setSecondOrganization(matcher.group("separator"), matcher.group("secondOrganization"));
