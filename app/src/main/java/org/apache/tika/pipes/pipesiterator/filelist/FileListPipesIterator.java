...
public class FileListPipesIterator extends PipesIterator implements Initializable {
    ...
    public void checkInitialization(InitializableProblemHandler initializableProblemHandler) throws TikaConfigException {
        ...
        if (!Files.isRegularFile(path, new LinkOption[0])) {
            throw new TikaConfigException(OooOo.OooOO0O("file list ", this.fileList, " does not exist. Must specify an existing file"));
        }
    }
    ...
    public void enqueue() throws InterruptedException, TimeoutException, IOException {
        ...
        BufferedReader bufferedReaderNewBufferedReader = Files.newBufferedReader(path, StandardCharsets.UTF_8);
        try {
            if (this.hasHeader) {
