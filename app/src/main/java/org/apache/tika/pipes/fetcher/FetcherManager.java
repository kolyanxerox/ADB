        ...
        for (Fetcher fetcher : list) {
            ...
            if (name != null) {
                ...
                while (iCharCount < length) {
                    int iCodePointAt = name.codePointAt(iCharCount);
                    if (Character.isWhitespace(iCodePointAt)) {
                        iCharCount += Character.charCount(iCodePointAt);
                    } else {
                        if (this.fetcherMap.containsKey(fetcher.getName())) {
                            throw new TikaConfigException(OooOo.OooOO0("Multiple fetchers cannot support the same prefix: ", fetcher.getName()));
                        }
