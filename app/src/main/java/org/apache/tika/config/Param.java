        ...
        for (Node firstChild = node.getFirstChild(); firstChild != null; firstChild = firstChild.getNextSibling()) {
            if (firstChild.getNodeType() == 1) {
                ...
                if (((Map) ((Param) param).actualValue).containsKey(localName)) {
                    throw new TikaConfigException(OooOo.OooOO0("Duplicate keys are not allowed: ", localName));
                }
                ...
                if ("params".equals(nodeItem.getLocalName())) {
                    ...
                    for (int i2 = 0; i2 < childNodes2.getLength(); i2++) {
                        if ("param".equals(childNodes2.item(i2).getLocalName())) {
                            ...
                            String strOooOO0O = OooOo.OooOO0O("set", paramLoad.getName().substring(0, 1).toUpperCase(Locale.US), paramLoad.getName().substring(1));
                            try {
                                try {
                                    ((Param) param).actualValue.getClass().getMethod(strOooOO0O, paramLoad.getType()).invoke(((Param) param).actualValue, paramLoad.getValue());
                                } catch (IllegalAccessException | InvocationTargetException e) {
                                    throw new TikaConfigException(OooOo.OooOO0("can't set param value: ", paramLoad.getName()), e);
                                }
                            } catch (NoSuchMethodException e2) {
                                throw new TikaConfigException(OooOo.OooOO0("can't find method: ", strOooOO0O), e2);
                            }
                        }
                    }
                }
    ...
    public static <T> Param<T> load(Node node) throws IllegalAccessException, NoSuchMethodException, DOMException, SecurityException, TikaConfigException, IllegalArgumentException, InvocationTargetException {
        ...
        if (namedItem4 != null) {
            try {
                ...
                throw new TikaConfigException(OooOo.OooOO0("can't find class: ", namedItem4.getTextContent()), e);
            }
        } else {
            cls = null;
        }
        ...
        if (namedItem2 != null) {
            ...
            if (((Param) param).type == null) {
                throw new TikaConfigException(OooOo.OooOO0("Must specify a \"type\" in: ", node.getLocalName()));
            }
        }
        if (cls != null) {
