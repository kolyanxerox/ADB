...
public class NetworkParser implements Parser {
    ...
    public static class ParsingTask implements Runnable {
        ...
        public void run() throws IOException {
            try {
                try {
                    o00O00.OooO0O0(this.input, this.output);
                } finally {
                    this.output.close();
                }
            } catch (Exception e) {
    ...
    private void parse(TikaInputStream tikaInputStream, ContentHandler contentHandler, Metadata metadata, ParseContext parseContext) throws TikaException, SAXException, IOException {
        ...
        try {
            new ParsingTask(tikaInputStream, uRLConnectionOpenConnection.getOutputStream()).parse(new o00Oo00.OooO00o(inputStream), contentHandler, metadata, parseContext);
            if (inputStream != null) {
                inputStream.close();
            }
        } catch (Throwable th3) {
