...
public class FlutterView extends FrameLayout implements MouseCursorPlugin.MouseCursorViewDelegate, KeyboardManager.ViewDelegate {
    ...
    private void init() {
        Log.m13463v(TAG, "Initializing FlutterView");
        if (this.flutterSurfaceView != null) {
            Log.m13463v(TAG, "Internally using a FlutterSurfaceView.");
            addView(this.flutterSurfaceView);
        } else if (this.flutterTextureView != null) {
            Log.m13463v(TAG, "Internally using a FlutterTextureView.");
            addView(this.flutterTextureView);
        } else {
            Log.m13463v(TAG, "Internally using a FlutterImageView.");
    ...
    private void sendViewportMetricsToFlutter() {
        if (!isAttachedToFlutterEngine()) {
            Log.m13465w(TAG, "Tried to send viewport metrics from Android to Flutter but this FlutterView was not attached to a FlutterEngine.");
            return;
        }
    ...
    public void attachToFlutterEngine(@NonNull FlutterEngine flutterEngine) {
        Log.m13463v(TAG, "Attaching to a FlutterEngine: " + flutterEngine);
        if (isAttachedToFlutterEngine()) {
            if (flutterEngine == this.flutterEngine) {
                Log.m13463v(TAG, "Already attached to this engine. Doing nothing.");
                return;
            } else {
                Log.m13463v(TAG, "Currently attached to a different engine. Detaching and then attaching to new engine.");
        ...
        try {
            ...
            Log.m13459e(TAG, "TextServicesManager not supported by device, spell check disabled.");
        }
    ...
    public ZeroSides calculateShouldZeroSides() {
        ...
        if (context.getResources().getConfiguration().orientation == 2) {
            int rotation = ((DisplayManager) context.getSystemService(C3034d9.h.f8099d)).getDisplay(0).getRotation();
            if (rotation == 1) {
    ...
    public void detachFromFlutterEngine() {
        Log.m13463v(TAG, "Detaching from a FlutterEngine: " + this.flutterEngine);
        if (!isAttachedToFlutterEngine()) {
            Log.m13463v(TAG, "FlutterView not attached to an engine. Not detaching.");
            return;
        }
        Iterator<FlutterEngineAttachmentListener> it = this.flutterEngineAttachmentListeners.iterator();
        while (it.hasNext()) {
    ...
    public final WindowInsets onApplyWindowInsets(@NonNull WindowInsets windowInsets) {
        ...
        if (i >= 28 && (displayCutout = windowInsets.getDisplayCutout()) != null) {
            for (Rect rect : displayCutout.getBoundingRects()) {
                Log.m13463v(TAG, "DisplayCutout area reported with bounds = " + rect.toString());
    ...
    public void onAttachedToWindow() {
        ...
        this.windowInfoListener = new com.applovin.impl.sdk.p005ad.OooO00o(this, 2);
    ...
    public void onConfigurationChanged(@NonNull Configuration configuration) {
        ...
        if (this.flutterEngine != null) {
            Log.m13463v(TAG, "Configuration changed. Sending locales and user settings to Flutter.");
    ...
    public void onSizeChanged(int i, int i2, int i3, int i4) {
        ...
        StringBuilder sbOooOo0o = OooOo.OooOo0o("Size changed. Sending Flutter new viewport metrics. FlutterView was ", i3, " x ", i4, ", it is now ");
        sbOooOo0o.append(i);
        sbOooOo0o.append(" x ");
        sbOooOo0o.append(i2);
    ...
    public void revertImageView(@NonNull final Runnable runnable) {
        if (this.flutterImageView == null) {
            Log.m13463v(TAG, "Tried to revert the image view, but no image view is used.");
            return;
        }
        RenderSurface renderSurface = this.previousRenderSurface;
        if (renderSurface == null) {
            Log.m13463v(TAG, "Tried to revert the image view, but no previous surface was used.");
            return;
        }
        this.renderSurface = renderSurface;
        this.previousRenderSurface = null;
    ...
    */
    ...
    public void setWindowInfoListenerDisplayFeatures(WindowLayoutInfo windowLayoutInfo) {
        ...
        for (DisplayFeature displayFeature : displayFeatures) {
            Log.m13463v(TAG, "WindowInfoTracker Display Feature reported with bounds = " + displayFeature.getBounds().toString() + " and type = " + displayFeature.getClass().getSimpleName());
    ...
    public FlutterView(@NonNull Context context, @NonNull RenderMode renderMode) {
        ...
        this.systemSettingsObserver = new ContentObserver(new Handler(Looper.getMainLooper())) { // from class: io.flutter.embedding.android.FlutterView.2
            ...
            public void onChange(boolean z) {
                ...
                Log.m13463v(FlutterView.TAG, "System settings changed. Sending user settings to Flutter.");
