...
public class KeyboardManager implements InputConnectionAdaptor.KeyboardDelegate, KeyboardChannel.KeyboardMethodHandler {
    ...
    public void onUnhandled(@NonNull KeyEvent keyEvent) {
        ...
        if (this.redispatchedEvents.remove(keyEvent)) {
            Log.m13465w(TAG, "A redispatched key event was consumed before reaching KeyboardManager");
        }
    }
    ...
    public void destroy() {
        ...
        if (size > 0) {
            Log.m13465w(TAG, "A KeyboardManager was destroyed with " + String.valueOf(size) + " unhandled redispatch event(s).");
        }
    }
