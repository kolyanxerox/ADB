...
public class PlatformViewWrapper extends FrameLayout {
    ...
    public void draw(Canvas canvas) {
        ...
        if (platformViewRenderTarget == null) {
            super.draw(canvas);
            Log.m13459e(TAG, "Platform view cannot be composed without a RenderTarget.");
            return;
        }
        Surface surface = platformViewRenderTarget.getSurface();
        if (!surface.isValid()) {
            Log.m13459e(TAG, "Platform view cannot be composed without a valid RenderTarget surface.");
            return;
        }
