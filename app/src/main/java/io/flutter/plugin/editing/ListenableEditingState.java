...
class ListenableEditingState extends SpannableStringBuilder {
    ...
    public void addEditingStateListener(EditingStateWatcher editingStateWatcher) {
        if (this.mChangeNotificationDepth > 0) {
            Log.m13459e(TAG, "adding a listener " + editingStateWatcher.toString() + " in a listener callback");
        }
        if (this.mBatchEditNestDepth <= 0) {
            this.mListeners.add(editingStateWatcher);
        } else {
            Log.m13465w(TAG, "a listener was added to EditingState while a batch edit was in progress");
    ...
    public void beginBatchEdit() {
        ...
        if (this.mChangeNotificationDepth > 0) {
            Log.m13459e(TAG, "editing state should not be changed in a listener callback");
        }
    ...
    public void endBatchEdit() {
        ...
        if (i == 0) {
            Log.m13459e(TAG, "endBatchEdit called without a matching beginBatchEdit");
            return;
        }
        if (i == 1) {
            ...
            if (!this.mListeners.isEmpty()) {
                Log.m13463v(TAG, "didFinishBatchEdit with " + String.valueOf(this.mListeners.size()) + " listener(s)");
    ...
    public void removeEditingStateListener(EditingStateWatcher editingStateWatcher) {
        if (this.mChangeNotificationDepth > 0) {
            Log.m13459e(TAG, "removing a listener " + editingStateWatcher.toString() + " in a listener callback");
        }
