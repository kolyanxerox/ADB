...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public class C06751 implements ThreadUtil.MainThreadCallback<T> {
        ...
        private Runnable mMainThreadRunnable = new Runnable() { // from class: androidx.recyclerview.widget.MessageThreadUtil.1.1
            ...
            public void run() {
                SyncQueueItem next = C06751.this.mQueue.next();
                while (next != null) {
                    int i = next.what;
                    if (i == 1) {
                        C06751.this.val$callback.updateItemCount(next.arg1, next.arg2);
                    } else if (i == 2) {
                        C06751.this.val$callback.addTile(next.arg1, (TileList.Tile) next.data);
                    } else if (i != 3) {
                        Log.e("ThreadUtil", "Unsupported message, what=" + next.what);
                    } else {
                        C06751.this.val$callback.removeTile(next.arg1, next.arg2);
                    }
                    next = C06751.this.mQueue.next();
                }
            }
        };

        public C06751(ThreadUtil.MainThreadCallback mainThreadCallback) {
    ...
    public class C06762 implements ThreadUtil.BackgroundCallback<T> {
        ...
        private Runnable mBackgroundRunnable = new Runnable() { // from class: androidx.recyclerview.widget.MessageThreadUtil.2.1
            ...
            public void run() {
                while (true) {
                    SyncQueueItem next = C06762.this.mQueue.next();
                    if (next == null) {
                        C06762.this.mBackgroundRunning.set(false);
                        return;
                    }
                    int i = next.what;
                    if (i == 1) {
                        C06762.this.mQueue.removeMessages(1);
                        C06762.this.val$callback.refresh(next.arg1);
                    } else if (i == 2) {
                        C06762.this.mQueue.removeMessages(2);
                        C06762.this.mQueue.removeMessages(3);
                        C06762.this.val$callback.updateRange(next.arg1, next.arg2, next.arg3, next.arg4, next.arg5);
                    } else if (i == 3) {
                        C06762.this.val$callback.loadTile(next.arg1, next.arg2);
                    } else if (i != 4) {
                        Log.e("ThreadUtil", "Unsupported message, what=" + next.what);
                    } else {
                        C06762.this.val$callback.recycleTile((TileList.Tile) next.data);
                    }
                }
            }
        };

        public C06762(ThreadUtil.BackgroundCallback backgroundCallback) {
    ...
    public ThreadUtil.BackgroundCallback<T> getBackgroundProxy(ThreadUtil.BackgroundCallback<T> backgroundCallback) {
        return new C06762(backgroundCallback);
    }

    @Override // androidx.recyclerview.widget.ThreadUtil
    public ThreadUtil.MainThreadCallback<T> getMainThreadProxy(ThreadUtil.MainThreadCallback<T> mainThreadCallback) {
        return new C06751(mainThreadCallback);
    }
