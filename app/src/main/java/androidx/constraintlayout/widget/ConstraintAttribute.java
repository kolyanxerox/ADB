...
public class ConstraintAttribute {
    ...
    public static /* synthetic */ class C01871 {
        ...
        static {
            ...
            try {
                f52x66adad53[AttributeType.COLOR_DRAWABLE_TYPE.ordinal()] = 2;
            } catch (NoSuchFieldError unused2) {
            }
            try {
                f52x66adad53[AttributeType.INT_TYPE.ordinal()] = 3;
            } catch (NoSuchFieldError unused3) {
            }
            try {
                f52x66adad53[AttributeType.FLOAT_TYPE.ordinal()] = 4;
            } catch (NoSuchFieldError unused4) {
            }
            try {
                f52x66adad53[AttributeType.STRING_TYPE.ordinal()] = 5;
            } catch (NoSuchFieldError unused5) {
            }
            try {
                f52x66adad53[AttributeType.BOOLEAN_TYPE.ordinal()] = 6;
            } catch (NoSuchFieldError unused6) {
            }
            try {
                f52x66adad53[AttributeType.DIMENSION_TYPE.ordinal()] = 7;
    ...
    public static void parse(Context context, XmlPullParser xmlPullParser, HashMap<String, ConstraintAttribute> map) {
        ...
        Object string;
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(xmlPullParser), C0189R.styleable.CustomAttribute);
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            if (index == C0189R.styleable.CustomAttribute_attributeName) {
                string2 = typedArrayObtainStyledAttributes.getString(index);
                if (string2 != null && string2.length() > 0) {
                    string2 = Character.toUpperCase(string2.charAt(0)) + string2.substring(1);
                }
            } else if (index == C0189R.styleable.CustomAttribute_customBoolean) {
                objValueOf = Boolean.valueOf(typedArrayObtainStyledAttributes.getBoolean(index, false));
                attributeType2 = AttributeType.BOOLEAN_TYPE;
            } else {
                if (index == C0189R.styleable.CustomAttribute_customColorValue) {
                    attributeType = AttributeType.COLOR_TYPE;
                    string = Integer.valueOf(typedArrayObtainStyledAttributes.getColor(index, 0));
                } else if (index == C0189R.styleable.CustomAttribute_customColorDrawableValue) {
                    attributeType = AttributeType.COLOR_DRAWABLE_TYPE;
                    string = Integer.valueOf(typedArrayObtainStyledAttributes.getColor(index, 0));
                } else if (index == C0189R.styleable.CustomAttribute_customPixelDimension) {
                ...
                } else if (index == C0189R.styleable.CustomAttribute_customDimension) {
                    attributeType = AttributeType.DIMENSION_TYPE;
                    string = Float.valueOf(typedArrayObtainStyledAttributes.getDimension(index, 0.0f));
                } else if (index == C0189R.styleable.CustomAttribute_customFloatValue) {
                    attributeType = AttributeType.FLOAT_TYPE;
                    string = Float.valueOf(typedArrayObtainStyledAttributes.getFloat(index, Float.NaN));
                } else if (index == C0189R.styleable.CustomAttribute_customIntegerValue) {
                    attributeType = AttributeType.INT_TYPE;
                    string = Integer.valueOf(typedArrayObtainStyledAttributes.getInteger(index, -1));
                } else if (index == C0189R.styleable.CustomAttribute_customStringValue) {
    ...
    public static void setAttributes(View view, HashMap<String, ConstraintAttribute> map) throws IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
        ...
        for (String str : map.keySet()) {
            ConstraintAttribute constraintAttribute = map.get(str);
            String strOooOO0 = OooOo.OooOO0("set", str);
            try {
                int i = C01871.f52x66adad53[constraintAttribute.mType.ordinal()];
                Class cls2 = Integer.TYPE;
                Class cls3 = Float.TYPE;
                switch (i) {
                    case 1:
                        cls.getMethod(strOooOO0, cls2).invoke(view, Integer.valueOf(constraintAttribute.mColorValue));
                        break;
                    case 2:
                        Method method = cls.getMethod(strOooOO0, Drawable.class);
                        ColorDrawable colorDrawable = new ColorDrawable();
                        colorDrawable.setColor(constraintAttribute.mColorValue);
                        method.invoke(view, colorDrawable);
                        break;
                    case 3:
                        cls.getMethod(strOooOO0, cls2).invoke(view, Integer.valueOf(constraintAttribute.mIntegerValue));
                        break;
                    case 4:
                        cls.getMethod(strOooOO0, cls3).invoke(view, Float.valueOf(constraintAttribute.mFloatValue));
                        break;
                    case 5:
                        cls.getMethod(strOooOO0, CharSequence.class).invoke(view, constraintAttribute.mStringValue);
                        break;
                    case 6:
                        cls.getMethod(strOooOO0, Boolean.TYPE).invoke(view, Boolean.valueOf(constraintAttribute.mBooleanValue));
                        break;
                    case 7:
                        cls.getMethod(strOooOO0, cls3).invoke(view, Float.valueOf(constraintAttribute.mFloatValue));
                        break;
                ...
                StringBuilder sbOooOoO0 = OooOo.OooOoO0(" Custom Attribute \"", str, "\" not found on ");
                sbOooOoO0.append(cls.getName());
                Log.e(TAG, sbOooOoO0.toString());
                e.printStackTrace();
            } catch (NoSuchMethodException e2) {
                Log.e(TAG, e2.getMessage());
                Log.e(TAG, " Custom Attribute \"" + str + "\" not found on " + cls.getName());
                Log.e(TAG, cls.getName() + " must have a method " + strOooOO0);
            } catch (InvocationTargetException e3) {
                StringBuilder sbOooOoO02 = OooOo.OooOoO0(" Custom Attribute \"", str, "\" not found on ");
                sbOooOoO02.append(cls.getName());
                Log.e(TAG, sbOooOoO02.toString());
                e3.printStackTrace();
            }
        }
    }
    ...
    public boolean diff(ConstraintAttribute constraintAttribute) {
        ...
        if (constraintAttribute != null && (attributeType = this.mType) == constraintAttribute.mType) {
            switch (C01871.f52x66adad53[attributeType.ordinal()]) {
                case 1:
                case 2:
    ...
    public float getValueToInterpolate() {
        switch (C01871.f52x66adad53[this.mType.ordinal()]) {
            case 1:
            case 2:
    ...
    public int noOfInterpValues() {
        int i = C01871.f52x66adad53[this.mType.ordinal()];
        return (i == 1 || i == 2) ? 4 : 1;
    }
    ...
    public void setInterpolatedValue(View view, float[] fArr) {
        ...
        try {
            ...
            StringBuilder sbOooOoO0 = OooOo.OooOoO0("cannot access method ", str, "on View \"");
            sbOooOoO0.append(Debug.getName(view));
            sbOooOoO0.append("\"");
            Log.e(TAG, sbOooOoO0.toString());
            e.printStackTrace();
        } catch (NoSuchMethodException e2) {
            StringBuilder sbOooOoO02 = OooOo.OooOoO0("no method ", str, "on View \"");
            sbOooOoO02.append(Debug.getName(view));
            sbOooOoO02.append("\"");
            Log.e(TAG, sbOooOoO02.toString());
            e2.printStackTrace();
        } catch (InvocationTargetException e3) {
