...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f126mX = new float[iMin];
        this.f127mY = new float[iMin];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < iMin; i2++) {
            pathMeasure.getPosTan((i2 * length) / (iMin - 1), fArr, null);
            this.f126mX[i2] = fArr[0];
            this.f127mY[i2] = fArr[1];
        }
        if (Math.abs(this.f126mX[0]) <= 1.0E-5d && Math.abs(this.f127mY[0]) <= 1.0E-5d) {
            int i3 = iMin - 1;
            if (Math.abs(this.f126mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f127mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < iMin) {
                    float[] fArr2 = this.f126mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        sb.append(this.f126mX[0]);
        sb.append(",");
        sb.append(this.f127mY[0]);
        sb.append(" end:");
        int i6 = iMin - 1;
        sb.append(this.f126mX[i6]);
        sb.append(",");
        sb.append(this.f127mY[i6]);
    ...
    private void parseInterpolatorFromTypeArray(TypedArray typedArray, XmlPullParser xmlPullParser) {
        if (TypedArrayUtils.hasAttribute(xmlPullParser, "pathData")) {
            ...
            if (pathCreatePathFromPathData == null) {
                throw new InflateException(OooOo.OooOO0("The path is null, which is created from ", namedString));
            }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f126mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f126mX[i2]) {
        ...
        float[] fArr = this.f126mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f127mY[i];
        }
        float f5 = (f - f3) / f4;
        float[] fArr2 = this.f127mY;
        float f6 = fArr2[i];
        return OooOo.OooO00o(fArr2[length], f6, f5, f6);
    }
