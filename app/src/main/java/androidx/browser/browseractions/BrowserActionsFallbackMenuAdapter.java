...
class BrowserActionsFallbackMenuAdapter extends BaseAdapter {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) throws Resources.NotFoundException {
        ...
        if (view == null) {
            view = LayoutInflater.from(this.mContext).inflate(C0127R.layout.browser_actions_context_menu_row, (ViewGroup) null);
            ImageView imageView = (ImageView) view.findViewById(C0127R.id.browser_actions_menu_item_icon);
            TextView textView = (TextView) view.findViewById(C0127R.id.browser_actions_menu_item_text);
        ...
        if (browserActionItem.getIconUri() != null) {
            final OooOOO oooOOOLoadBitmap = BrowserServiceFileProvider.loadBitmap(this.mContext.getContentResolver(), browserActionItem.getIconUri());
            oooOOOLoadBitmap.addListener(new Runnable() { // from class: androidx.browser.browseractions.BrowserActionsFallbackMenuAdapter.1
                @Override // java.lang.Runnable
                public void run() {
                    Bitmap bitmap;
                    if (TextUtils.equals(title, viewHolderItem.mText.getText())) {
                        try {
                            bitmap = (Bitmap) oooOOOLoadBitmap.get();
