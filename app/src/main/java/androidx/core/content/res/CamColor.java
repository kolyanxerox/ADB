...
public class CamColor {
    ...
    /* renamed from: mJ */
    private final float f58mJ;
    private final float mJstar;

    /* renamed from: mM */
    private final float f59mM;

    /* renamed from: mQ */
    private final float f60mQ;

    /* renamed from: mS */
    private final float f61mS;
    ...
    public static void fromColorInViewingConditions(int i, ViewingConditions viewingConditions, float[] fArr, float[] fArr2) {
        ...
        float fPow = (float) Math.pow((Math.abs(f9) * viewingConditions.getFl()) / 100.0d, 0.42d);
        float fPow2 = (float) Math.pow((Math.abs(f10) * viewingConditions.getFl()) / 100.0d, 0.42d);
        float fPow3 = (float) Math.pow((Math.abs(f11) * viewingConditions.getFl()) / 100.0d, 0.42d);
        ...
        double d = fSignum3;
        float f12 = ((float) (((fSignum2 * (-12.0d)) + (fSignum * 11.0d)) + d)) / 11.0f;
    ...
    private static CamColor fromJchInFrame(float f, float f2, float f3, ViewingConditions viewingConditions) {
        ...
        return new CamColor(f3, f2, f, aw, flRoot, fSqrt, f4, ((float) Math.cos(d)) * fLog, fLog * ((float) Math.sin(d)));
    }
    ...
    public int viewed(ViewingConditions viewingConditions) {
        ...
        float fl = (100.0f / viewingConditions.getFl()) * Math.signum(f5) * ((float) Math.pow((float) Math.max(0.0d, (Math.abs(f5) * 27.13d) / (400.0d - Math.abs(f5))), 2.380952380952381d));
        float fl2 = (100.0f / viewingConditions.getFl()) * Math.signum(f6) * ((float) Math.pow((float) Math.max(0.0d, (Math.abs(f6) * 27.13d) / (400.0d - Math.abs(f6))), 2.380952380952381d));
        float fl3 = (100.0f / viewingConditions.getFl()) * Math.signum(f7) * ((float) Math.pow((float) Math.max(0.0d, (Math.abs(f7) * 27.13d) / (400.0d - Math.abs(f7))), 2.380952380952381d));
